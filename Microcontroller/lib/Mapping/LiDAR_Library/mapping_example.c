...
static void mapping_data_received(const mapping_measurement_t *measurement)
{
    ...
    if (measurement->valid) {
        ...
        if (last_timestamp > 0) {
            time_diff = (measurement->timestamp_us - last_timestamp) / 1000; // Convert to ms
        }
        
        ESP_LOGI(TAG, "[#%lu] New measurement (Δt=%lums): Angle=%d°, Distance=%dmm, Quality=%d%%, Signal=%d",
                measurement_count,
                time_diff,
        ...
        ESP_LOGW(TAG, "[#%lu] Invalid measurement received", measurement_count);
    }
}
...
void mapping_example_calibration(void)
{
    ...
    mapping_config_t custom_config = {
        ...
        .motion_compensation_enabled = false, // Enable motion compensation
